...
import 'package:kazakhi_auto_web/constants/app_constants.dart';
import 'package:kazakhi_auto_web/utils/local_utils.dart';
...
class ApiClient {
  ...
  static Future<dynamic> getBinary(String endpoint) async {
    String token = await LocalUtils.getAccessToken() ?? '';
    final url = Uri.parse(AppConstant.baseUrl.toString() + endpoint);
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      await _refreshToken(response);
      return await getBinary(endpoint);
    } else {
      return {
        'success': false,
